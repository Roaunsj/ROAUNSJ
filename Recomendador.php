...
(stristr($micadena,'video_collection_matematicas') == FALSE))
  ...
  if ($haycontenido == 0) {
  //drupal_set_message("Informacion insuficiente para recomendaciones por contenido");
cache_set('recomendacontenido','0');
  }else
      {
//drupal_set_message("Visitas " . $max ."Pagina " . $maxurl);
//Aca debes extraer de $maxurl la cadena islandora PID y luego buscar ese objeto con islandora solr search, extraerle los datos que buscamos y llamar a una busqueda con esa cadena.
...
//drupal_set_message("objetomax " . $objetomax);
//drupal_set_message("Visitas " . $max ." Objeto " . $objetomax);
cache_set('recomendacontenido','1');
cache_set('objetoporcontenido',$objetomax);
      }
/*************************************************************************/
//drupal_set_message("Su area es " . $account->profile_areaint);
//drupal_set_message("Su categoria es " . $account->profile_categoria);
...
variable_set($recomenda,'4');//seteamos la variable global para las recomendaciones
